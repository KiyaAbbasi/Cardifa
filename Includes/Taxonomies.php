...
function cardifa_register_taxonomies() {
    ...
    $args_cat = array(
        ...
        'public'            => true,
        'show_ui'           => true,
        'show_in_menu'      => false,           // زیرمجموعه‌ی CPT کارت
        'show_in_rest'      => true,
        'rest_base'         => 'cardifa_card_category',
        'rewrite'           => array( 'slug' => 'cardifa-category', 'with_front' => false ),
        'show_admin_column' => true,
    );
    register_taxonomy( 'cardifa_card_category', array( 'cardifa' ), $args_cat );
    ...
    $args_tag = array(
        ...
        'rewrite'           => array( 'slug' => 'cardifa-tag', 'with_front' => false ),
        'show_admin_column' => true,
    );
    register_taxonomy( 'cardifa_card_tag', array( 'cardifa' ), $args_tag );
}
add_action( 'init', 'cardifa_register_taxonomies', 0 );

/**
 * ۲) افزودن فیلد «تصویر شاخص» به فرم افزودن دسته/برچسب
 */
// نمایش فیلد در فرم افزودن
...
function cardifa_taxonomy_custom_column( $content, $column, $term_id ) {
    if ( 'thumbnail' === $column ) {
        ...
        if ( $url ) {
            $content = '<img src="' . esc_url( $url ) . '" alt="" style="max-width:60px;height:auto;">';
        } else {
            $content = '&mdash;';
        }
    }
...
function cardifa_enqueue_taxonomy_media_script( $hook ) {
    // فقط در صفحات مربوط به افزودن/ویرایش ترم کارت
    if ( 'edit-tags.php' === $hook && in_array( get_current_screen()->taxonomy, array( 'cardifa_card_category','cardifa_card_tag' ), true ) ) {
        wp_enqueue_media();
        wp_enqueue_script(
