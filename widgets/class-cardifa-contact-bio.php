...
class contact_bio extends \Elementor\Widget_Base {
    ...
    public function get_title() {
        return esc_html__( 'تماس با من', BIO_PLUGIN_NAME );
    }
	...
	protected function register_content_section_2() {
		...
		$this->add_control(
			'socials_item',
			[ 
				'label' => esc_html__( 'شبکه های اجتماعی', BIO_PLUGIN_NAME ),
	...
	protected function register_content_section_3() {
		...
		$this->register_text_style( 'name-top', '.name-top', $algin = false );
                     ...
                     foreach ( $settings['socials_item'] as $index => $item ) :
                        ...
                        <div class="<?php echo esc_html($post_item_cls); ?>">
                            <article <?php post_class( array( 'post-wrapper', 'cart-hover-' . $settings['hover'] ) ); ?>>
								<p class="name-top">
									<?php echo esc_html( $item['title-top'] ); ?>
								</p>
